...
describe('test topic api', () => {
    ...
    describe('test create method', () => {
        const topicCreationRequest = async (topic: Record<string, string | null | number>) => 
    ...
    describe('test list method', () => {
        const topicListRequest = async (searchParams?: URLSearchParams) => 
            await request(app).get(`/api/v1/topics/?${searchParams ? searchParams.toString() : ''}`).send()
            ...
            describe('test pagination', () => {
                it('should return first two topics', async () => {
                    ...
                    const received = JSON.stringify({
                        count,
                        rows: rows.map(
                            (row: Record<string, string>) => ({ name: row.name })
                        )
                    })
    ...
    describe('test update method', () => {
        const topicUpdateRequest = async (id: number, data: Record<string, string | number>) => 
    ...
    describe('test patch method', () => {
        const topicPatchRequest = async (id: number, data: Record<string, string | number>) => 
