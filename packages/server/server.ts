...
import { CLIENT_DIR, DIST_DIR, DIST_SSR_DIR } from './assets/dir'
  ...
  app.use('*', async (req, res, next) => {
    ...
    try {
      ...
      if (ENVS.__DEV__) {
        ...
        render = (await vite.ssrLoadModule(path.resolve(CLIENT_DIR, 'ssr.tsx')))
          .render
      } else {
        template = fs.readFileSync(
        ...
        render = (await import(path.resolve(DIST_SSR_DIR, 'ssr.cjs'))).render
      }
